...
**Optional TX-confirm gate before seeding (`CHUNK_BROADCAST_TX_CONFIRM_GATE`, default OFF):** a chunk is only accepted by a node that already knows the tx's `data_root`, and the bundle tx header is posted ~0.38s before its chunks — so under slow propagation the first chunk POSTs 400 ("rejected chunk") and only land via retries. When the gate is ON, `seed-bundle` waits until the tx is confirmed network-wide (mined) before staging + broadcasting chunks, eliminating that race. It's a BullMQ re-queue (not a blocking sleep) hard-capped by `CHUNK_BROADCAST_GATE_MAX_MS` (default 10 min) so a slow tx still broadcasts and bundles never strand. While gating, the bundle stays in `posted_bundle` and the `seed-bundle` queue shows delayed jobs — both normal. Metric `chunk_broadcast_gate_total{result}` is on **upload-workers** (not the API). See `chunkBroadcastGate.ts` and PR #152.
...
**Workers**: PM2-managed in `packages/upload-service/src/workers/allWorkers.ts` (fork mode - single instance). **Seven** queues have env-tunable concurrency (the rest are hardcoded in `allWorkers.ts`):
- `PLAN_WORKER_CONCURRENCY` (default **1** — the plan handler is a self-draining loop, and the internal scheduler fires plan-bundle on a wall-clock tick, so concurrency 1 is the overlap guard; raising it re-introduces overlap), `PREPARE_WORKER_CONCURRENCY` (default 3), `POST_WORKER_CONCURRENCY` (default 2), `VERIFY_WORKER_CONCURRENCY` (default 3), `OPTICAL_WORKER_CONCURRENCY` (default 5), `BROADCAST_CHUNKS_WORKER_CONCURRENCY` (default 10 — chunks are small + independent, so this is the highest), `ARCHIVE_COPY_WORKER_CONCURRENCY` (default 3)
- Hardcoded: seed=2, put-offsets=5, new-data-item=5, unbundle-bdi=2, finalize-upload=3, cleanup-fs=1, redrive-posted=1, refund-balance=3, ensure-partitions=1

...
- **Chunk-broadcast TX-confirm gate (opt-in, `CHUNK_BROADCAST_TX_CONFIRM_GATE=true`; default OFF).** When ON, `seed-bundle` holds a bundle's chunk broadcast until its tx is confirmed network-wide (mined → `data_root` known), so chunks can't lose the propagation race that causes `broadcast-chunks` HTTP 400s. It's a BullMQ re-queue (the seed job re-delays itself every `CHUNK_BROADCAST_GATE_POLL_MS`, default 5s), hard-capped by `CHUNK_BROADCAST_GATE_MAX_MS` (default 10 min, keep **below** `POSTED_STALE_THRESHOLD_MS` 30 min). **Don't misread the gate as a stall:** while gating, a bundle legitimately sits in `posted_bundle` for ~1–2 min AND the `upload-seed-bundle` queue shows elevated **delayed** jobs (one per gated bundle, re-firing every 5s) — both are NORMAL when the gate is on, not a stuck stage. Confirm via `pm2 logs upload-workers` for `Chunk-broadcast gate: TX not yet confirmed; re-enqueued` (holding) → `gate cleared … reason: confirmed` (released). ⚠️ The gate metric `chunk_broadcast_gate_total{result=confirmed|requeued|cap_reached}` is emitted by **upload-workers, not the API** — it is NOT on `:3001/bundler_metrics`. A sustained `cap_reached` rate means txs aren't confirming within the cap (chunks still broadcast at the cap). **On this dev box the gate is currently ON** (validated 2026-06-30, PR #152).
- **`redrive-posted` (#40) is the seed-failure safety net** — a bundle whose `seed-bundle` exhausts its retries used to be stranded forever in `posted_bundle`. The re-driver re-enqueues seeding for stale `posted_bundle` rows (`POSTED_STALE_THRESHOLD_MS`, default 30 min) and after `MAX_SEED_REDRIVES` (default 5) demotes to `failed_bundle`, emitting `posted_bundle_failed_to_seed_total`. `posted_bundle` is no longer a dead-end. (With the gate ON, a bundle in `posted_bundle` for **less than the gate cap** is just gating, not a redrive candidate — only rows past `POSTED_STALE_THRESHOLD_MS` are.)

...
- [ ] DNS: `upload.<domain>` + `payment.<domain>` → this box's public IP. (Bundler `:3001`/`:4001` stay **localhost-only** per §2 — nginx proxies from `127.0.0.1`.)
- [ ] Install `infrastructure/nginx/ar-io-bundler.conf` (sites-available → sites-enabled); replace `<domain>`; `nginx -t` → reload.

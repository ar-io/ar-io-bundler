...
> **Optional: dedicated archive read tier (two-tier MinIO).** On an SSD+HDD box you can run a
> second, archive MinIO that mirrors served content (raw data items + bundle payloads)
> and takes **all** gateway reads, keeping the bundler MinIO reserved for the bundling
> pipeline. Point the gateway's `AWS_ENDPOINT` at the archive MinIO (port **9002**) instead of
> the bundler MinIO. This is opt-in and gated on `ARCHIVE_*` env (default off = single-MinIO
> behavior, unchanged). See `docs/architecture/TWO_TIER_MINIO.md` and the
